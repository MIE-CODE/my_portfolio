...
export const CanvasPreview = ({ imageSrc }: CanvasPreviewProps) => {
  ...
  const [useIframe, setUseIframe] = useState(true); // Default to iframe since it's more reliable
  ...
  useEffect(() => {
    if (imageSrc && imageSrc !== "#") {
      ...
      const url = getScreenshotUrl(imageSrc, serviceIndex);
      setScreenshotUrl(url);
      setError(false);
    } else {
      setError(true);
    }
  }, [imageSrc, serviceIndex]);
